    ...
    return () => (
      <div style={styles.emptyPage}>
        <div style={styles.emptyCard}>
          ...
          <button type="button" onClick={props.onBack} style={styles.secondaryButton}>חזרה</button>
        </div>
      </div>
    );
  },
...
const ClientDashboardPage = defineComponent({
  ...
  props: {
    reportData: { default: null },
    onBack: { type: Function, default: () => {} },
    onCreateShareLink: { type: Function, default: () => {} },
    isSharedMode: { default: false },
    sharePayload: { default: null },
    viewMode: { default: "family" },
    selectedMemberId: { default: null },
    onChangeView: { type: Function, default: () => {} },
    onOpenPreviousReports: { type: Function, default: () => {} },
  },
  setup(props) {
    const activeSection = ref("pension");
    const selectedPieSegment = ref(null);
    const initialScopeId = props.viewMode === "member" && props.selectedMemberId ? props.selectedMemberId : "family";
    const localScopeId = ref(initialScopeId);

    const clientModel = computed(() => buildClientModelFromReportData(props.reportData));
    const detailedMembers = computed(() => buildDetailedMembers(props.reportData, clientModel.value));
    const specialSections = computed(() => buildSpecialSectionsModel(props.reportData));
    const navItems = computed(() => buildNavItems(specialSections.value));
    const scope = computed(() => getSelectedScope(clientModel.value, detailedMembers.value, localScopeId.value));

    const handleOpenPieDrawer = (payload) => {
      selectedPieSegment.value = {
        ...payload,
        details: buildPieSegmentDetails(scope.value, payload),
      };
    };

    const handleClosePieDrawer = () => {
      selectedPieSegment.value = null;
    };

    const handleScopeChange = (event) => {
      const nextScopeId = event.target.value;
      localScopeId.value = nextScopeId;
      if (nextScopeId === "family") props.onChangeView("family", null);
      else props.onChangeView("member", nextScopeId);
    };

    return () => {
      if (!props.reportData || !props.reportData.family) return <EmptyDashboardState onBack={props.onBack} />;
        ...
        <div class="client-web-shell">
          ...
          <aside class="client-sidebar">
            <div class="client-sidebar-brand">
              <ZviranMark />
              <div>
                <div class="client-brand-title">הפנסיה שלי</div>
            ...
            <nav class="client-sidebar-nav" aria-label="ניווט במסך הלקוח">
              {navItems.value.map((item) => (
                <button key={item.id} type="button" onClick={() => (activeSection.value = item.id)} class={activeSection.value === item.id ? "client-nav-item active" : "client-nav-item"}>
          ...
          <main class="client-main">
            <header class="client-topbar">
              <div class="client-topbar-title-wrap">
                ...
                <h1 class="client-page-title">דוח פנסיוני משפחתי מאוחד</h1>
                <div class="client-page-subtitle">{scope.value.isFamily ? "תצוגה מאוחדת לכל המשפחה" : `תצוגה אישית עבור ${scope.value.name}`}</div>
              </div>

              <div class="client-topbar-actions">
                <button type="button" class="client-history-button" onClick={props.onOpenPreviousReports} title="הכנה לצפייה בנתונים קודמים">
                  <span class="client-history-icon">↺</span>
                  <span><strong>נתונים קודמים</strong><small>הכנה לגרסאות דוח קודמות</small></span>
                </button>

                <div class="client-updated-box"><span>עודכן לאחרונה:</span><strong>{clientModel.value.lastUpdated || "—"}</strong></div>

                <label class="client-scope-select-wrap">
                  <span>תצוגה</span>
                  <select value={localScopeId.value} onChange={handleScopeChange} class="client-scope-select">
                    <option value="family">משפחה מאוחדת</option>
                    {detailedMembers.value.map((member) => <option key={member.id || member.name} value={member.id || member.name}>{member.name || "ללא שם"}</option>)}
                  </select>
                </label>

                {!props.isSharedMode ? <button type="button" onClick={props.onBack} class="client-back-button">חזרה ל־REPORT</button> : null}
              </div>
            </header>

            <section class="client-content-card">
              {activeSection.value === "pension" ? <PensionSection scope={scope.value} /> : null}
              {activeSection.value === "personal" ? <PersonalDetailsSection members={detailedMembers.value} /> : null}
              {activeSection.value === "allocation" ? <AllocationSection scope={scope.value} onSegmentClick={handleOpenPieDrawer} /> : null}
              {activeSection.value === "insurance" ? <InsuranceSection scope={scope.value} /> : null}
              {activeSection.value === "loans" ? <LoansSection scope={scope.value} /> : null}
              {activeSection.value === "capitalClassification" ? <CapitalClassificationSection sections={specialSections.value.capitalClassification} /> : null}
              {activeSection.value === "section28" ? <Section28Section section28Capping={specialSections.value.section28Capping} /> : null}
              {activeSection.value === "recognizedPension" ? <RecognizedPensionSection entries={specialSections.value.recognizedPensionEntries} /> : null}
              {activeSection.value === "summary" ? <ConversationSummarySection scope={scope.value} clientModel={clientModel.value} reportData={props.reportData} /> : null}
            </section>
          </main>

          <PieSegmentDrawer selected={selectedPieSegment.value} onClose={handleClosePieDrawer} />
        </div>
      );
    };
  },
...
const PensionSection = defineComponent({
  props: { scope: {} },
  setup(props) {
    return () => {
      const summary = props.scope.summary || {};
      return (
        <div>
          ...
          <div class="client-grid-2 client-margin-top">
            <ExposurePanel title="חשיפה מנייתית משוקללת" value={props.scope.exposures?.equity} description={getExposureLabel(props.scope.exposures?.equity)} />
            <ExposurePanel title='חשיפה לחו"ל' value={props.scope.exposures?.foreign} description={getForeignExposureLabel(props.scope.exposures?.foreign)} />
          </div>
        </div>
      );
    };
...
const PersonalDetailsSection = defineComponent({
  props: { members: {} },
  setup(props) {
    return () => {
      const displayMembers = safeArray(props.members).slice(0, 2);
      return (
        <div>
...
const PersonalDetailsCard = defineComponent({
  props: { member: {}, index: {} },
  setup(props) {
    return () => {
      const details = props.member?.personalDetails || {};
      const title = props.index === 0 ? "בעל" : props.index === 1 ? "אישה" : "בן/בת משפחה";
      return (
        <div class="client-personal-card">
          <div class="client-personal-card-header">
            <div class="client-personal-avatar">{String(details.name || props.member?.name || "?").trim().slice(0, 1)}</div>
            <div>
              <div class="client-personal-card-kicker">{title}</div>
              <h3>{details.name || props.member?.name || "ללא שם"}</h3>
            </div>
          </div>

          <div class="client-personal-fields">
            <PersonalField label="שם לקוח" value={details.name || props.member?.name || "—"} />
...
const PersonalField = defineComponent({
  props: { label: {}, value: {} },
  setup(props) {
    return () => (
...
const AllocationSection = defineComponent({
  props: { scope: {}, onSegmentClick: {} },
  setup(props) {
    return () => (
      <div>
        <SectionTitle
        ...
        <div class="client-allocation-top-pies">
          <DonutCard
            ...
            items={props.scope.distributions?.products || props.scope.productTypes || []}
            type="product"
            scope={props.scope}
          ...
          <DonutCard
            ...
            items={props.scope.distributions?.managers || props.scope.managers || []}
            type="manager"
            scope={props.scope}
            onSegmentClick={props.onSegmentClick}
          />
        ...
        <div class="client-main-groups-wide client-margin-top">
          <DonutCard
            ...
            items={props.scope.distributions?.mainGroups || []}
            type="mainGroup"
            scope={props.scope}
            onSegmentClick={props.onSegmentClick}
            wide
        ...
        <AssetProductTablesSection productTables={props.scope.assetProductTables} />
      </div>
    );
  },
});
...
const AssetProductTablesSection = defineComponent({
  props: { productTables: {} },
  setup(props) {
    const initialTables = safeArray(props.productTables);
    const openId = ref(initialTables[0]?.id || "");

    return () => {
      const tables = safeArray(props.productTables);
      ...
      return (
        <div class="client-asset-products client-margin-top">
          ...
          {tables.map((table) => {
            const isOpen = openId.value === table.id;
            return (
              <div key={table.id} class="client-product-accordion">
                <button type="button" class="client-product-summary" onClick={() => (openId.value = isOpen ? "" : table.id)}>
                  ...
                  <span class="client-product-strip-item"><small>תשואה 12<br />משוקללת</small><b>{formatSignedPercent(table.weightedReturn12)}</b></span>
                  <span class="client-product-strip-item"><small>תשואה 36<br />משוקללת</small><b>{formatSignedPercent(table.weightedReturn36)}</b></span>
                  <span class="client-product-strip-item"><small>תשואה 60<br />משוקללת</small><b>{formatSignedPercent(table.weightedReturn60)}</b></span>
                  <span class="client-product-strip-item"><small>סטיית תקן<br />36</small><b>{formatSignedPercent(table.weightedSt36)}</b></span>
                  <span class="client-product-strip-item"><small>שארפ<br />36</small><b>{formatDecimal(table.weightedSharp36, 2)}</b></span>
                </button>
...
const AssetProductRoutesTable = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => {
      const safeRows = safeArray(props.rows);
...
const InsuranceSection = defineComponent({
  props: { scope: {} },
  setup(props) {
    return () => {
      const insurance = props.scope.insurance || {};
      const deathCoverageRows = safeArray(props.scope.deathCoverageProducts);
      ...
      return (
        <div>
          ...
          <div class="client-panel client-margin-top">
            ...
            <InsuranceProductsTable rows={deathCoverageRows} isFamily={Boolean(props.scope.isFamily)} />
          </div>
        </div>
      );
    };
...
const InsuranceProductsTable = defineComponent({
  props: { rows: {}, isFamily: {} },
  setup(props) {
    return () => {
      const safeRows = safeArray(props.rows);
      ...
      return (
        <div class="client-table-wrap client-margin-top">
          <table class={props.isFamily ? "client-table client-insurance-table family" : "client-table client-insurance-table member"}>
            <colgroup>
              {columns.map((column) => (
...
const LoansSection = defineComponent({
  props: { scope: {} },
  setup(props) {
    return () => {
      const loans = safeArray(props.scope.loans?.details);
      ...
      const ratio = Number(props.scope.summary?.totalAssets || 0) > 0 ? (totalAmount / Number(props.scope.summary.totalAssets || 0)) * 100 : 0;
      return (
        <div>
...
const CapitalClassificationSection = defineComponent({
  props: { sections: {} },
  setup(props) {
    return () => {
      const capitalSections = safeArray(props.sections);
...
const CapitalKpi = defineComponent({
  props: { title: {}, value: {}, tone: { default: "default" } },
  setup(props) {
    return () => (
      <div class={`client-capital-kpi ${props.tone}`}>
...
const CapitalOwnerBlock = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const pensionRows = safeArray(props.section?.pensionPolicies);
      const studyRows = safeArray(props.section?.studyFunds);
      ...
      return (
        <div class="client-report-panel client-capital-owner-panel">
          <div class="client-report-table-heading">
            <div>
              <div class="client-report-panel-title">פירוק נכסים ללקוח {props.section?.ownerLabel || "—"}</div>
              <p>{props.section?.sourceFileName ? `מקור הנתונים: ${props.section.sourceFileName}` : "חלוקה לפי פוליסות, גמל, פנסיה וקרנות השתלמות."}</p>
            </div>
          </div>
                ...
                <table class="client-table client-capital-table">
                  ...
                  <tbody>
                    {pensionRows.map((row, index) => {
                      ...
                      return (
                        <Fragment key={key}>
                          <CapitalPensionRow row={row} index={index} isOpen={Boolean(openRows.value[key])} onToggle={() => toggleRow(row, index)} />
                          {openRows.value[key] ? <CapitalGroupedDetailsRow row={row} /> : null}
                        </Fragment>
                      );
                    })}
...
const CapitalPensionRow = defineComponent({
  props: { row: {}, index: { default: 0 }, isOpen: { default: false }, onToggle: { type: Function, default: () => {} } },
  setup(props) {
    return () => {
      const productType = getCapitalText(props.row, ["productType", "productGroup", "productName", "type", "מוצר / קבוצה", "סוג מוצר"]) || "—";
      const sourceRows = safeArray(props.row?.sourceRows);
      const hasDetails = sourceRows.length > 0;
      const capitalRewards = getCapitalNumber(props.row, ["capitalRewards", "תגמולים הוניים"]);
      const pre2000Rewards = getCapitalNumber(props.row, ["annuityRewardsUntil2000", "pre2000Rewards", "rewardsBefore2000", "תגמולים קצבתיים עד 1.1.2000"]);
      const previousEmployerContinuity = getCapitalNumber(props.row, ["previousEmployersSeveranceRightsSequence", "previousEmployerContinuity", "previousEmployerSeveranceRights", "פיצויים ממעסיקים קודמים ברצף זכויות"]);
      const currentEmployerTax = getCapitalNumber(props.row, ["currentEmployerSeveranceTaxable", "taxableCompensation", "currentEmployerCompensationTax", "פיצויים מעסיק נוכחי למס"]);
      const totalPension = getCapitalRowPensionValue(props.row);
      const totalCapital = getCapitalRowCapitalValue(props.row, false);

      return (
        <tr>
          <td class="client-capital-product-cell">
            {hasDetails ? (
              <button type="button" class="client-capital-expand-button" onClick={props.onToggle} aria-expanded={props.isOpen} title="הצגת התוכניות המקובצות">
...
const CapitalGroupedDetailsRow = defineComponent({
  props: { row: {} },
  setup(props) {
    return () => {
      const sourceRows = safeArray(props.row?.sourceRows);
...
const CapitalStudyRow = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const sourceRows = safeArray(props.row?.sourceRows);
      const hasDetails = sourceRows.length > 0;
      const policyNumber = getCapitalText(props.row, ["policyNumber", "fundNumber", "policyNo", "מספר קופה", "מספר פוליסה"]) || getCapitalText(props.row, ["productType", "productGroup", "productName", "type", "מוצר / קבוצה", "סוג מוצר"]) || "קרן השתלמות";
      const totalFund = getCapitalRowFundValue(props.row);
      ...
      return (
        <>
          <tr>
            <td class="client-capital-product-cell">
              {hasDetails ? (
                <button type="button" class="client-capital-expand-button" onClick={() => (isOpen.value = !isOpen.value)} aria-expanded={isOpen.value} title="הצגת הקרנות המקובצות">
          ...
          {isOpen.value ? <CapitalStudyDetailsRow row={props.row} /> : null}
        </>
      );
    };
  },
...
const CapitalStudyDetailsRow = defineComponent({
  props: { row: {} },
  setup(props) {
    return () => {
      const sourceRows = safeArray(props.row?.sourceRows);
...
const Section28Section = defineComponent({
  props: { section28Capping: {} },
  setup(props) {
    return () => {
      const entries = Array.isArray(props.section28Capping)
        ? props.section28Capping
        : props.section28Capping
        ? [props.section28Capping]
        : [];
...
const Section28CostSplit = defineComponent({
  props: { group: {} },
  setup(props) {
    return () => {
      const rows = safeArray(props.group?.rows).filter((row) => isMeaningfulSection28Value(row.value));
...
const Section28SavingSimulation = defineComponent({
  props: { group: {} },
  setup(props) {
    return () => {
      const rows = safeArray(props.group?.rows).filter((row) => isMeaningfulSection28Value(row.value));
...
const Section28RetirementSimulation = defineComponent({
  props: { group: {} },
  setup(props) {
    return () => {
      const rows = safeArray(props.group?.rows).filter((row) => isMeaningfulSection28Value(row.value));
...
const Section28GenericGroup = defineComponent({
  props: { group: {} },
  setup(props) {
    return () => {
      const rows = safeArray(props.group?.rows).filter((row) => isMeaningfulSection28Value(row.value));
      ...
      return (
        <div class="client-report-panel">
          <div class="client-report-panel-title">{props.group.title || "פירוט נוסף"}</div>
          {rows.map((row, index) => (
...
const Section28DataRow = defineComponent({
  props: { row: {}, forceHighlight: { default: false } },
  setup(props) {
    return () => {
      const isHighlighted = props.forceHighlight || isSection28ImportantRow(props.row.label);
      return (
        <div class={isHighlighted ? "client-section28-row highlighted" : "client-section28-row"}>
          <div class="client-section28-row-label">{props.row.label || "—"}</div>
          <div class="client-section28-row-value">{formatSection28DisplayValue(props.row.value)}</div>
        </div>
      );
    };
  },
...
const Section28KpiBox = defineComponent({
  props: { row: {}, highlight: { default: false } },
  setup(props) {
    return () => (
      <div class={props.highlight ? "client-report-kpi-box highlighted" : "client-report-kpi-box"}>
        <span>{props.row.label || "—"}</span>
        <strong>{formatSection28DisplayValue(props.row.value)}</strong>
      </div>
    );
  },
});

const Section28MonthlySavingRow = defineComponent({
  props: { row: {} },
  setup(props) {
    return () => (
...
const Section28EmptyNote = defineComponent({
  setup() {
    return () => <div class="client-empty-state compact">אין נתון להצגה</div>;
  },
});

const Section28ComparisonTable = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => (
      <div class="client-report-panel client-margin-top">
        <div class="client-report-panel-title">השוואה בין תרחישים</div>
        <div class="client-section28-comparison-layout">
          <div class="client-table-wrap">
            <table class="client-table client-section28-table">
              <thead>
                <tr>
                  <th>סעיף</th>
                  <th>לפני קיטום</th>
          ...
          <Section28ComparisonBars rows={props.rows} />
        </div>
      </div>
    );
  },
...
const Section28ComparisonBars = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => {
      const chartRows = props.rows.filter((row) => {
                ...
                {orderedBars.map((bar, barIndex) => (
                  <div key={barIndex} class="client-section28-bar-row">
                    ...
                    <div class="client-section28-bar-track">
                      <div class={`client-section28-bar-fill ${bar.className}`} style={{ width: `${Math.max((bar.value / rowMaxValue) * 100, bar.value ? 4 : 0)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
...
const RecognizedPensionSection = defineComponent({
  props: { entries: { default: () => [] } },
  setup(props) {
    return () => {
      const safeEntries = safeArray(props.entries).filter(hasRecognizedPensionRows);
              ...
              return (
                <div class="client-report-owner-block" key={`${entry?.owner || "owner"}-${entryIndex}`}>
                  ...
                  {vestedBalanceTable?.sourceFileName ? (
                    <div class="client-source-strip"><span>מקור נתונים: <strong>{vestedBalanceTable.sourceFileName}</strong></span></div>
                  ) : null}
...
const VestedPdfCalculationTable = defineComponent({
  props: { rows: {}, pdfTotal: {} },
  setup(props) {
    return () => {
      const columns = [
      ...
      return (
        <div class="client-report-panel">
          <div class="client-report-table-heading">
            ...
            <div class="client-report-pill">סה״כ תשלומים פטורים: {formatReportNumber(props.pdfTotal)}</div>
          </div>
          ...
          <div class="client-table-wrap">
            <table class="client-table client-vested-table">
              ...
              <tbody>
                ...
                <tr class="total-row">
                  {columns.map((column) => (
                    <td key={column.key}>{column.key === "fundName" ? "סה״כ טבלת PDF" : column.key === "exemptPayments" ? formatReportNumber(props.pdfTotal) : "—"}</td>
                  ))}
                </tr>
              </tbody>
            </table>
...
const ManualRecognizedPensionTable = defineComponent({
  props: { rows: {}, manualTotal: {} },
  setup(props) {
    return () => (
      <div class="client-report-panel client-margin-top">
        <div class="client-report-table-heading">
          ...
          <div class="client-report-pill gold">סה״כ קצבה מוכרת: {formatReportNumber(props.manualTotal)}</div>
        </div>
        ...
        <div class="client-table-wrap client-manual-table-wrap">
          <table class="client-table client-manual-recognized-table">
            ...
            <tbody>
              ...
              <tr class="total-row"><td>סה״כ</td><td>{formatReportNumber(props.manualTotal)}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
...
const TaxSavingGapSummary = defineComponent({
  props: { pdfTotal: {}, manualTotal: {} },
  setup(props) {
    return () => {
      const gap = props.pdfTotal - props.manualTotal;
      return (
...
const ConversationSummarySection = defineComponent({
  props: { scope: {}, clientModel: {}, reportData: {} },
  setup(props) {
    return () => {
      const savedSummary = props.reportData?.conversationSummary || props.reportData?.clientConversationSummary || props.clientModel?.conversationSummary || "";
      const savedActions = props.reportData?.actionRecommendations || props.reportData?.recommendationsText || props.clientModel?.actionRecommendations || "";
      const fallbackSummary = `כאן יוצג סיכום השיחה עם הלקוח עבור ${props.scope.name}. בשלב זה זהו אזור הכנה, וניתן לחבר אליו בהמשך שדה טקסט מה־REPORT או ממנגנון שמירת הדוח.`;
          ...
          <div class="client-grid-2">
            ...
            <TextPanel title="המלצות פעולה" text={savedActions || fallbackActions}>
              <div class="client-action-buttons">
                <button type="button" class="client-action-button primary" onClick={() => downloadActionsPdf(actionsForPdf)}>הורדת PDF</button>
...
const SectionTitle = defineComponent({
  ...
  setup(props) {
    return () => <div class="client-section-title-row"><div><h2>{props.title}</h2>{props.subtitle ? <p>{props.subtitle}</p> : null}</div></div>;
  },
});

const KpiCard = defineComponent({
  props: { icon: {}, title: {}, value: {}, subtext: {} },
  setup(props) {
    return () => <div class="client-kpi-card"><div class="client-kpi-icon">{props.icon}</div><div class="client-kpi-title">{props.title}</div><div class="client-kpi-value">{props.value}</div>{props.subtext ? <div class="client-kpi-sub">{props.subtext}</div> : null}</div>;
  },
});
...
const MetricBox = defineComponent({
  ...
  setup(props) {
    return () => <div class="client-metric-box"><div class="client-metric-icon">{props.icon}</div><div><div class="client-metric-title">{props.title}</div><div class="client-metric-value">{props.value}</div></div></div>;
  },
});

const ComparisonCard = defineComponent({
  props: { title: {}, explanation: {}, withValue: {}, withoutValue: {} },
  setup(props) {
    return () => {
      const withNum = Number(props.withValue || 0);
      const withoutNum = Number(props.withoutValue || 0);
      const maxValue = Math.max(withNum, withoutNum, 1);
      return <div class="client-panel"><h3>{props.title}</h3>{props.explanation ? <p class="client-panel-subtitle">{props.explanation}</p> : null}<CompareBar label="עם המשך הפקדות" value={withNum} maxValue={maxValue} primary /><CompareBar label="ללא המשך הפקדות" value={withoutNum} maxValue={maxValue} /></div>;
    };
  },
});

const CompareBar = defineComponent({
  props: { label: {}, value: {}, maxValue: {}, primary: { default: false } },
  setup(props) {
    return () => {
      const ratio = Math.max((Number(props.value || 0) / Number(props.maxValue || 1)) * 100, props.value ? 5 : 0);
      return <div class="client-compare-row"><div class="client-compare-top"><span>{props.label}</span><strong>{formatCurrency(props.value)}</strong></div><div class="client-compare-track"><div class={props.primary ? "client-compare-fill primary" : "client-compare-fill muted"} style={{ width: `${ratio}%` }} /></div></div>;
    };
  },
});

const ExposurePanel = defineComponent({
  props: { title: {}, value: {}, description: {} },
  setup(props) {
    return () => {
      const safe = Math.max(0, Math.min(100, Number(props.value || 0)));
      return <div class="client-panel"><div class="client-exposure-top"><div><h3>{props.title}</h3><p class="client-panel-subtitle">{props.description}</p></div><strong>{formatPercent(safe)}</strong></div><div class="client-exposure-track"><div class="client-exposure-fill" style={{ width: `${safe}%` }} /></div><div class="client-exposure-scale"><span>0%</span><span>25%</span><span>50%</span><span>75%</span><span>100%</span></div></div>;
    };
  },
});

const DonutCard = defineComponent({
  props: { title: {}, items: {}, wide: { default: false }, type: { default: "segment" }, scope: { default: null }, onSegmentClick: {} },
  setup(props) {
    return () => {
      const segments = buildSegments(props.items);
      return (
        <div class={props.wide ? "client-panel client-donut-panel wide" : "client-panel client-donut-panel"}>
          <h3>{props.title}</h3>

          {segments.length ? (
            <div class={props.wide ? "client-donut-layout wide" : "client-donut-layout"}>
              <InteractiveDonut
                title={props.title}
                segments={segments}
                type={props.type}
              ...
              <div class="client-legend">
                {segments.slice(0, 7).map((seg) => (
                  <button
                    ...
                    onClick={() =>
                      typeof props.onSegmentClick === "function"
                        ? props.onSegmentClick({ title: props.title, type: props.type, segment: seg, scope: props.scope })
                        : null
                    }
                    title={`${seg.name} · ${Math.round(seg.percent)}% · ${formatCurrency(seg.value)}`}
                  >
                    <span class="client-legend-dot" style={{ background: seg.color }} />
...
const InteractiveDonut = defineComponent({
  props: { title: {}, segments: {}, type: {}, scope: {}, onSegmentClick: {}, wide: { default: false } },
  setup(props) {
    return () => {
      const size = props.wide ? 190 : 154;
      const strokeWidth = props.wide ? 36 : 30;
      const radius = (size - strokeWidth - 12) / 2;
      const center = size / 2;
      const circumference = 2 * Math.PI * radius;
      const clickable = typeof props.onSegmentClick === "function";

      return (
        <svg
          class={props.wide ? "client-donut-svg wide" : "client-donut-svg"}
          width={size}
          height={size}
...
const PieSegmentDrawer = defineComponent({
  props: { selected: {}, onClose: {} },
  setup(props) {
    return () => {
      if (!props.selected) return null;

      const segment = props.selected.segment || {};
      const details = safeArray(props.selected.details);
      const total = Number(segment.value || 0);
      const typeTitle =
        props.selected.type === "product"
          ? "פירוט לפי מוצר"
          : props.selected.type === "manager"
          ? "פירוט לפי גוף מנהל"
          : props.selected.type === "mainGroup"
          ? "פירוט לפי אפיק ראשי"
          : "פירוט";

      return (
        <div class="client-drawer-overlay" onClick={props.onClose}>
          <aside class="client-drawer" onClick={(event) => event.stopPropagation()}>
            <div class="client-drawer-header">
              <button type="button" class="client-drawer-close" onClick={props.onClose}>×</button>
              <div>
...
const TextPanel = defineComponent({
  props: { title: {}, text: {} },
  setup(props, { slots }) {
    return () => <div class="client-panel"><h3>{props.title}</h3><div class="client-text-panel">{props.text}</div>{slots.default?.()}</div>;
  },
});
...
const ZviranMark = defineComponent({
  setup() {
...
function PiggyIcon() { return <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="M4 13C4 9.7 6.9 7 10.8 7H15.3C18.1 7 20 8.9 20 11.6V17H17.4L16.7 19H8.2L7.5 17H4V13Z" stroke="#00215D" stroke-width="2" stroke-linejoin="round"/><path d="M8 7L6.7 4.8H10.3L11.4 7" stroke="#FF2756" stroke-width="2" stroke-linejoin="round"/><path d="M16.2 10.8H16.25" stroke="#FF2756" stroke-width="3" stroke-linecap="round"/></svg>; }
function GrowthIcon() { return <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="M4 18V9" stroke="#00215D" stroke-width="2.2" stroke-linecap="round"/><path d="M10 18V5" stroke="#00215D" stroke-width="2.2" stroke-linecap="round"/><path d="M16 18V12" stroke="#00215D" stroke-width="2.2" stroke-linecap="round"/><path d="M3 19H21" stroke="#FF2756" stroke-width="2.2" stroke-linecap="round"/></svg>; }
function WalletIcon() { return <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><rect x="4" y="7" width="16" height="11" rx="3" stroke="#00215D" stroke-width="2.2"/><path d="M17 12H20V16H17C15.9 16 15 15.1 15 14C15 12.9 15.9 12 17 12Z" stroke="#FF2756" stroke-width="2.2"/><path d="M7 7L15 4" stroke="#00215D" stroke-width="2.2" stroke-linecap="round"/></svg>; }
...
function ShieldIcon() { return <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="M12 4L19 7V12C19 16.6 16.1 19.4 12 21C7.9 19.4 5 16.6 5 12V7L12 4Z" stroke="#00215D" stroke-width="2.2" stroke-linejoin="round"/><path d="M9 12L11 14L15.5 9.5" stroke="#FF2756" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>; }
function PersonIcon() { return <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="7.5" r="3.5" stroke="#00215D" stroke-width="2.2"/><path d="M5 20C5.7 16.7 8.2 14.8 12 14.8C15.8 14.8 18.3 16.7 19 20" stroke="#FF2756" stroke-width="2.2" stroke-linecap="round"/></svg>; }
