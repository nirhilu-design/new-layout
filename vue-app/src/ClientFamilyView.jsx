...
const ClientFamilyView = defineComponent({
  ...
  setup(props) {
    const linkedReportData = ref(readLinkedReportData());
    const linkedClientModel = ref(readLinkedClientModel());
    const selectedPieSegment = ref(null);
    const activeClientTab = ref("overview");

    const syncReportData = (event) => {
      if (event?.detail?.reportData || event?.detail?.clientModel) {
        if (event.detail.reportData) linkedReportData.value = event.detail.reportData;
        if (event.detail.clientModel) linkedClientModel.value = event.detail.clientModel;
        return;
      }

      linkedReportData.value = readLinkedReportData();
      linkedClientModel.value = readLinkedClientModel();
    };
    ...
    const model = computed(() => linkedClientModel.value || props.clientModel || {});
    const sourceReportData = computed(
      () => linkedReportData.value || model.value.sourceReportData || props.clientModel?.sourceReportData || {}
    );

    const summary = computed(() => model.value.summary || {});
    const exposures = computed(() => model.value.exposures || {});
    const members = computed(() => model.value.members || []);
    const managers = computed(() => model.value.distributions?.managers || []);
    const products = computed(() => model.value.distributions?.products || []);
    const mainGroups = computed(
      () =>
        model.value.distributions?.mainGroups ||
        model.value.distributions?.mainGroupAllocation ||
        model.value.mainGroupAllocation ||
        model.value.distributions?.assetClasses ||
        []
    );

    const loans = computed(() => model.value.loans || {});
    const loanDetails = computed(() => (Array.isArray(loans.value.details) ? loans.value.details : []));

    const conversationSummaryText = computed(
      () =>
        sourceReportData.value?.conversationSummary ||
        sourceReportData.value?.clientConversationSummary ||
        sourceReportData.value?.summaryText ||
        model.value?.conversationSummary ||
        ""
    );

    const actionRecommendationsText = computed(
      () =>
        sourceReportData.value?.actionRecommendations ||
        sourceReportData.value?.clientActionRecommendations ||
        sourceReportData.value?.recommendationsText ||
        sourceReportData.value?.recommendations ||
        model.value?.actionRecommendations ||
        model.value?.recommendationsText ||
        ""
    );
    ...
    const openPieDrawer = (payload) => {
      selectedPieSegment.value = {
        ...
        details: buildPieSegmentDetails(payload, {
          sourceReportData: sourceReportData.value,
          model: model.value,
          members: members.value,
          products: products.value,
          managers: managers.value,
          mainGroups: mainGroups.value,
        }),
      };
    };
    ...
    const section28Capping = computed(
      () => props.clientModel.section28Capping || sourceReportData.value.section28Capping || null
    );
    const hasSection28Data = computed(
      () => Array.isArray(section28Capping.value?.groups) && section28Capping.value.groups.length > 0
    );

    const vestedBalanceTable = computed(
      () => props.clientModel.vestedBalanceTable || sourceReportData.value.vestedBalanceTable || null
    );
    const recognizedPensionAdjustments = computed(
      () =>
        props.clientModel.recognizedPensionAdjustments ||
        sourceReportData.value.recognizedPensionAdjustments ||
        []
    );
    const hasVestedBalanceData = computed(
      () =>
        (Array.isArray(vestedBalanceTable.value?.rows) && vestedBalanceTable.value.rows.length > 0) ||
        (Array.isArray(recognizedPensionAdjustments.value) && recognizedPensionAdjustments.value.length > 0)
    );

    const capitalClassification = computed(() =>
      normalizeClientCapitalClassification(
        sourceReportData.value?.capitalClassification ||
          model.value?.capitalClassification ||
          props.clientModel?.capitalClassification ||
          null
      )
    );

    const hasCapitalClassificationData = computed(() =>
      capitalClassification.value.some(
        (section) => section.pensionPolicies.length > 0 || section.studyFunds.length > 0
      )
    );
    ...
    watch(
      ...
      () => {
        if (!hasCapitalClassificationData.value && activeClientTab.value === "capital") {
          activeClientTab.value = "overview";
        }
      },
      { immediate: true }
    );

    const summaryText = computed(() => getClientConversationSummaryText(model.value, sourceReportData.value));
    const recommendationsText = computed(() => getClientRecommendationsText(model.value, sourceReportData.value));
    ...
    const totalLoansAmount = computed(() =>
      loanDetails.value.reduce((sum, loan) => sum + Number(loan.amount || loan.balance || 0), 0)
    );

    const loanRatioToAssets = computed(() =>
      Number(summary.value.totalAssets || 0) > 0
        ? (totalLoansAmount.value / Number(summary.value.totalAssets || 0)) * 100
        : 0
    );

    const lumpBars = computed(() =>
      buildCompareBars(
        summary.value.projectedLumpSumWithDeposits,
        summary.value.projectedLumpSumWithoutDeposits,
        formatCurrency
      )
    );

    const pensionBars = computed(() =>
      buildCompareBars(
        summary.value.monthlyPensionWithDeposits,
        summary.value.monthlyPensionWithoutDeposits,
        formatCurrency
      )
    );

    return () => (
      <div class="client-family-root" style={page}>
        <style>
          {`
        ...
        <div class="family-client-shell">
          <aside class="family-client-sidebar no-print" aria-label="תפריט אזורי הדוח">
            ...
            <button
              type="button"
              class={`family-client-side-tab ${activeClientTab.value === "overview" ? "active" : ""}`}
              onClick={() => (activeClientTab.value = "overview")}
            >
              <span class="family-client-side-icon">⌂</span>
              <span>סקירה משפחתית</span>
            </button>

            {hasCapitalClassificationData.value ? (
              <button
                type="button"
                class={`family-client-side-tab ${activeClientTab.value === "capital" ? "active" : ""}`}
                onClick={() => (activeClientTab.value = "capital")}
              >
                <span class="family-client-side-icon">₪</span>
          ...
          <main class="family-client-content">
            <div class={`family-tab-panel ${activeClientTab.value === "overview" ? "active" : ""}`}>
          ...
          <Header
            ...
            lastUpdated={model.value.lastUpdated}
          />

          <section class="family-top-grid" style={topGrid}>
            <KpiCard
              icon={<GiftIcon />}
              title="סך נכסים"
              value={formatCurrency(summary.value.totalAssets)}
            ...
            <KpiCard
              icon={<DepositIcon />}
              title="הפקדה חודשית"
              value={formatCurrency(summary.value.monthlyDeposits)}
              subtext="סך ההפקדות החודשיות של בני המשפחה"
            />
          </section>

          <section class="family-wide-donut-grid" style={wideDonutGrid}>
            <DonutSummaryCard
          ...
          <section class="family-compare-grid" style={compareGrid}>
            <ComparisonChartCard
            ...
            <ComparisonChartCard
              ...
              bars={pensionBars.value}
            />
          </section>

          {hasSection28Data.value ? (
            <SectionCard title="קיטום על פי סעיף 28" icon="§">
              <div class="family-explanation" style={explanation}>
                אזור זה מוצג רק כאשר קיימים נתוני קיטום סעיף 28 שהועברו מה־REPORT.
              </div>

              <ClientSection28CappingSection data={section28Capping.value} />
            </SectionCard>
          ) : null}
        </div>

        <div class="family-print-page family-print-page-2">
          {hasVestedBalanceData.value ? (
            <SectionCard title="קצבה מוכרת" icon="📋">
              ...
              <ClientVestedBalanceSection
                table={vestedBalanceTable.value}
                adjustments={recognizedPensionAdjustments.value}
              />
            </SectionCard>
          ) : null}
          <section class="family-lower-grid" style={lowerTwoGrid}>
            <SectionCard title='חשיפה לחו"ל' icon="🌍">
              <ExposureMetricBlock
                value={exposures.value.foreign}
                valueText={formatPercent(exposures.value.foreign)}
                label={getForeignExposureLabel(exposures.value.foreign)}
            ...
            <SectionCard title="חשיפה מנייתית משוקללת" icon="📊">
              <ExposureMetricBlock
                value={exposures.value.equity}
                valueText={formatPercent(exposures.value.equity)}
                label={getExposureLabel(exposures.value.equity)}
          ...
          <SectionCard title="חלוקה לפי אפיקים ראשיים" icon="🥧">
            ...
            <FullWidthDonutCard
              items={mainGroups.value.length ? mainGroups.value : products.value}
              formatCurrency={formatCurrency}
          ...
          <SectionCard title="פירוט לפי בני משפחה" icon="👨‍👩‍👧‍👦">
            ...
            {members.value.length ? (
              <div class="family-members-grid" style={membersGrid}>
            ...
            {hasCapitalClassificationData.value ? (
              <div class={`family-tab-panel ${activeClientTab.value === "capital" ? "active" : ""}`}>
                <div class="family-print-page family-print-page-capital">
                  <SectionCard title="פירוק נכסים" icon="📑">
                    ...
                    <ClientCapitalClassificationSection
                      sections={capitalClassification.value}
                      formatCurrency={formatCurrency}
                    />
                  </SectionCard>
                </div>
          ...
          <SectionCard title="הלוואות על חשבון מוצרים פנסיוניים" icon="💳">
            ...
            {loanDetails.value.length ? (
              <>
                <div class="family-summary-grid" style={summaryStatsGrid}>
                  <SmallStat
                    title="סה״כ הלוואות"
                    value={formatCurrency(totalLoansAmount.value)}
                  />
                  <SmallStat
                    title="יחס לנכסים"
                    value={`${loanRatioToAssets.value.toFixed(1)}%`}
                  />
                </div>
                ...
                <div style={tableWrap}>
                  <table style={table}>
                    ...
                    <tbody>
                      {loanDetails.value.map((loan, index) => (
          ...
          <SectionCard title="סיכום מהיר" icon="🧾">
            <div class="family-summary-grid" style={summaryStatsGrid}>
              <SmallStat title="מוצרים" value={products.value.length} />
              <SmallStat title="גופים מנהלים" value={managers.value.length} />
              <SmallStat title="בני משפחה" value={members.value.length} />
              <SmallStat
                title="חשיפה מנייתית"
                value={formatPercent(exposures.value.equity)}
              />
            </div>

            <InfoBox
              label="קצבה חודשית צפויה"
              value={formatCurrency(summary.value.monthlyPensionWithDeposits)}
            />

            <InfoBox
              label="צבירה צפויה בגיל פרישה"
              value={formatCurrency(summary.value.projectedLumpSumWithDeposits)}
            />
          ...
          <SectionCard title="סיכום והמלצות" icon="🧾">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 14 }}>
              <div>
                ...
                {summaryText.value ? (
                  <ReadOnlyRecommendations text={summaryText.value} />
                ) : (
              ...
              <div>
                ...
                {recommendationsText.value ? (
                  <>
                    <ReadOnlyRecommendations text={recommendationsText.value} />
                    <div style={recommendationsActionsRow} class="no-print">
                      <button
                        type="button"
                        style={recommendationsPrimaryButton}
                        onClick={() => downloadOperationalActionsPdf(recommendationsText.value)}
                      >
                        הורדת PDF
                      </button>
                      <button
        ...
        <PieSegmentDrawer
          segment={selectedPieSegment.value}
...
const ClientCapitalClassificationSection = defineComponent({
  props: { sections: {}, formatCurrency: {} },
  setup(props) {
    return () => {
      ...
      return (
        <div style={clientCapitalWrap}>
          <div style={clientCapitalKpiGrid}>
            <ClientCapitalKpi title="סה״כ קופה" value={props.formatCurrency(totalFund)} />
            <ClientCapitalKpi title="סה״כ תגמולים" value={props.formatCurrency(totals.totalRewards)} />
            <ClientCapitalKpi title="סה״כ פיצויים" value={props.formatCurrency(totals.totalSeverance)} />
            <ClientCapitalKpi title="סה״כ הון" value={props.formatCurrency(totals.totalCapital + studyTotal)} tone="capital" />
            <ClientCapitalKpi title="סה״כ קצבה" value={props.formatCurrency(totals.totalPension)} tone="pension" />
          </div>
...
const ClientCapitalKpi = defineComponent({
  props: { title: {}, value: {}, tone: {} },
  setup(props) {
    return () => {
      const style = {
        ...clientCapitalKpi,
        ...(props.tone === "capital" ? clientCapitalKpiCapital : {}),
        ...(props.tone === "pension" ? clientCapitalKpiPension : {}),
      };

      return (
        <div style={style}>
          <div style={clientCapitalKpiLabel}>{props.title}</div>
          <div style={clientCapitalKpiValue}>{props.value}</div>
        </div>
      );
    };
  },
...
const ClientCapitalLegendItem = defineComponent({
  props: { color: {}, border: {}, label: {}, text: {} },
  setup(props) {
    return () => (
      <div style={clientCapitalLegendItem}>
        <span style={{ ...clientCapitalLegendSwatch, background: props.color, borderColor: props.border }} />
        <div>
          <div style={clientCapitalLegendLabel}>{props.label}</div>
          <div style={clientCapitalLegendText}>{props.text}</div>
        </div>
      </div>
    );
  },
...
const ClientCapitalPensionTable = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => {
...
const ClientCapitalStudyFundsTable = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => {
      const safeRows = Array.isArray(props.rows) ? props.rows : [];
...
const ClientSection28CappingSection = defineComponent({
  props: { data: {} },
  setup(props) {
    return () => {
      ...
      const comparisonRows = Array.isArray(props.data?.comparisonRows) ? props.data.comparisonRows : [];
...
const ClientSection28SideBox = defineComponent({
  props: { title: {}, rows: {}, summaryRows: {} },
  setup(props) {
    return () => {
      const allRows = [...(Array.isArray(props.rows) ? props.rows : []), ...(Array.isArray(props.summaryRows) ? props.summaryRows : [])];

      return (
        <div style={section28SideBox}>
          <div style={section28SideTitle}>{props.title}</div>

          {allRows.length ? (
            allRows.map((row, index) => (
              <div
                key={`${row.label}-${index}`}
                style={index >= (props.rows?.length || 0) ? section28RowHighlight : section28Row}
              >
...
const ClientSection28SimpleGroup = defineComponent({
  props: { title: {}, group: {} },
  setup(props) {
    return () => {
      const rows = Array.isArray(props.group?.rows)
        ? props.group.rows.filter((row) => isSection28DisplayValue(row?.value)).slice(0, 6)
        : [];
...
const ClientSection28ComparisonTable = defineComponent({
  props: { rows: {} },
  setup(props) {
    return () => (
...
const ReadOnlyRecommendations = defineComponent({
  props: { text: {} },
  setup(props) {
    return () => <div style={recommendationsReadOnlyBox}>{props.text}</div>;
  },
});
...
const PieSegmentDrawer = defineComponent({
  props: { segment: {}, onClose: {}, formatCurrency: {} },
  setup(props) {
    return () => {
      if (!props.segment) return null;

      const selected = props.segment.segment || {};
      const details = Array.isArray(props.segment.details) ? props.segment.details : [];
      const totalValue = Number(selected.value || 0);
      const typeLabel =
        props.segment.type === "product"
          ? "מוצר"
          : props.segment.type === "manager"
          ? "גוף מנהל"
          : props.segment.type === "mainGroup"
          ? "אפיק השקעה"
          : "נתון";

      return (
        <div style={drawerOverlay} onClick={props.onClose}>
          <aside style={drawerPanel} onClick={(event) => event.stopPropagation()}>
            <div style={drawerHeader}>
              <button type="button" onClick={props.onClose} style={drawerCloseButton}>
                ×
              </button>
              ...
              <div>
                ...
                <div style={drawerSub}>
                  {Math.round(Number(selected.percent || 0))}% מתוך הדוח · {props.formatCurrency(totalValue)}
                </div>
              </div>
            </div>
            ...
            <div style={drawerStatsGrid}>
              <div style={drawerStat}>
                ...
                <div style={drawerStatValue}>{props.formatCurrency(totalValue)}</div>
              </div>
                ...
                <tbody>
                  {details.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td style={drawerTd}>{props.formatCurrency(row.value)}</td>
...
const ClientVestedBalanceSection = defineComponent({
  props: { table: {}, adjustments: {} },
  setup(props) {
    return () => {
      const pdfRows = Array.isArray(props.table?.rows) ? props.table.rows : [];
      const manualRows = getClientManualRecognizedPensionRows(props.adjustments);
...
const ClientVestedPdfTable = defineComponent({
  props: { rows: {}, pdfTotal: {} },
  setup(props) {
    return () => {
      const columns = [
                  ...
                  {columns.map((column) => (
                    <td key={column.key} style={clientVestedTotalTd}>
                      {column.key === "fundName"
                        ...
                        ? formatClientReportNumber(props.pdfTotal)
                        : "—"}
                    </td>
                  ))}
                </tr>
...
const ClientManualPensionTable = defineComponent({
      ...
      <div style={{ marginTop: 20 }}>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={clientVestedTotalTd}>
                  {formatClientReportNumber(props.manualTotal)}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-start" }}>
          <div style={clientManualSummaryPill}>
            סה"כ קצבה מוכרת: {formatClientReportNumber(props.manualTotal)}
          </div>
        </div>
      </div>
    );
...
const ClientTaxSavingGapSummary = defineComponent({
...
const Header = defineComponent({
  props: { eyebrow: {}, title: {}, subtitle: {}, lastUpdated: {} },
  setup(props) {
    return () => (
      <section class="family-hero" style={heroHeader}>
        <div style={heroLogoWrap}>
          <ZviranLogo light />
        </div>

        <div style={heroCenter}>
          <div style={heroEyebrow}>{props.eyebrow}</div>
          <h1 style={heroTitle}>{props.title}</h1>
          <div style={heroSubtitle}>{props.subtitle}</div>
        </div>

        <div style={heroMeta}>
          <div style={heroMetaLabel}>תאריך עדכון</div>
          <div style={heroMetaValue}>
            {props.lastUpdated || new Intl.DateTimeFormat("he-IL").format(new Date())}
          </div>
        </div>
      </section>
    );
...
const KpiCard = defineComponent({
  ...
  setup(props) {
    return () => (
      <div class="family-kpi-card" style={kpiCard}>
        <div style={kpiIconWrap}>{props.icon}</div>
        <div style={kpiTitle}>{props.title}</div>
        <div class="family-kpi-value" style={kpiValue}>
          {props.value}
        </div>
        <div style={kpiSub}>{props.subtext}</div>
      </div>
    );
  },
});
...
const SectionCard = defineComponent({
  ...
  setup(props, { slots }) {
    return () => (
      <section class="family-section-card" style={sectionCard}>
...
const ExposureMetricBlock = defineComponent({
  props: { value: {}, valueText: {}, label: {}, explanationText: {} },
  setup(props) {
    return () => (
      <>
        <div class="family-explanation" style={explanation}>
        ...
        <div style={equityValueWrap}>
          <div style={equityValue}>{props.valueText}</div>
          <div style={equityLabel}>{props.label}</div>
        </div>

        <ModernBar value={props.value} />
      </>
    );
  },
});
...
const ComparisonChartCard = defineComponent({
  props: { title: {}, explanation: {}, bars: {} },
  setup(props) {
    return () => (
      <section class="family-compare-card" style={compareCard}>
        <div style={compareTitle}>{props.title}</div>
        <div style={compareDesc}>{props.explanation}</div>
...
const DonutSummaryCard = defineComponent({
  props: {
    ...
    drawerType: { default: "segment" },
    onSegmentClick: {},
  },
  setup(props) {
    return () => {
      const data = buildSegments(props.items);

      return (
        <section class="family-donut-card" style={donutCard}>
          <h3 style={donutTitle}>{props.title}</h3>
          <div style={{ ...smallText, marginTop: 6 }}>{props.subtitle}</div>

          {!data.segments.length ? (
            <EmptyText>אין נתונים להצגה</EmptyText>
          ) : (
            <div style={props.wide ? wideDonutLayout : donutLayout}>
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <DonutVisual
                  gradient={data.gradient}
                  segments={data.segments}
                  size={props.wide ? 182 : 122}
                  holeInset="31%"
                  soft
                  title={props.title}
              ...
              <div style={props.wide ? wideLegendList : legendList}>
                {data.segments.slice(0, props.wide ? 8 : 5).map((seg, index) => (
                  <LegendRow
...
const FullWidthDonutCard = defineComponent({
  props: {
    items: {},
    formatCurrency: {},
    emptyText: {},
    drawerType: { default: "mainGroup" },
    onSegmentClick: {},
  },
  setup(props) {
    return () => {
      ...
      return (
        <div class="family-main-breakdown" style={mainBreakdownCardLayout}>
          ...
          <div style={mainLegendWrap}>
            {data.segments.map((seg, index) => (
              ...
              >
                ...
                <div style={mainLegendValue}>{props.formatCurrency(seg.value)}</div>
...
const DonutVisual = defineComponent({
  props: {
    gradient: {},
    segments: { default: () => [] },
    size: { default: 110 },
    holeInset: { default: "31%" },
  ...
  setup(props) {
    return () => {
      ...
      const holeInset = props.holeInset;
...
const LegendRow = defineComponent({
  props: { seg: {}, formatCurrency: {}, wide: { type: Boolean, default: false } },
  setup(props) {
    return () => (
      <div style={props.wide ? wideLegendRow : legendRow}>
        <span style={{ ...legendDot, background: props.seg.color }} />
        <div style={{ minWidth: 0 }}>
          <div style={legendName}>{props.seg.name}</div>
          <div style={legendSub}>{props.formatCurrency(props.seg.value)}</div>
        </div>
        <div style={legendPercent}>{Math.round(props.seg.percent)}%</div>
      </div>
    );
  },
});
...
const MemberCard = defineComponent({
  props: { member: {}, formatCurrency: {} },
  setup(props) {
    return () => {
      const summary = props.member.summary || {};
      const insurance = props.member.insurance || {};

      return (
        <div class="family-member-card" style={memberCard}>
          <div style={memberTop}>
            <div>
              <div style={memberName}>{props.member.name}</div>
            </div>

            <div style={chip}>
              הפקדה חודשית: {props.formatCurrency(summary.monthlyDeposits)}
            </div>
          </div>

          <div style={centerCard}>
            <div style={centerLabel}>סך צבירה</div>
            <div class="family-center-value" style={centerValue}>
              {props.formatCurrency(summary.totalAssets)}
            </div>
          </div>
          ...
          <div style={miniGrid}>
            <CompareMiniCard
              ...
              leftLabel="עם הפקדות"
              leftValue={props.formatCurrency(summary.monthlyPensionWithDeposits)}
              rightLabel="ללא הפקדות"
              rightValue={props.formatCurrency(summary.monthlyPensionWithoutDeposits)}
            />

            <CompareMiniCard
              title="סכום חד הוני לפרישה"
              leftLabel="עם הפקדות"
              leftValue={props.formatCurrency(summary.projectedLumpSumWithDeposits)}
              rightLabel="ללא הפקדות"
              rightValue={props.formatCurrency(summary.projectedLumpSumWithoutDeposits)}
            />
          </div>
          ...
          <div style={insuranceGrid}>
            <div style={insuranceCard}>
              ...
              <div style={insuranceValue}>
                {props.formatCurrency(insurance.deathCoverage)}
              </div>
            </div>
            ...
            <div style={insuranceCard}>
              ...
              <div style={insuranceValue}>
                {props.formatCurrency(insurance.disabilityValue)} (
...
const CompareMiniCard = defineComponent({
  props: { title: {}, leftLabel: {}, leftValue: {}, rightLabel: {}, rightValue: {} },
  setup(props) {
    return () => (
      <div style={compareMiniCard}>
        <div style={compareMiniTitle}>{props.title}</div>

        <div style={compareMiniInner}>
          <div style={compareMiniSide}>
            <div style={compareMiniSideLabel}>{props.leftLabel}</div>
            <div style={compareMiniSideValue}>{props.leftValue}</div>
          </div>

          <div style={dividerLine} />

          <div style={compareMiniSide}>
            <div style={compareMiniSideLabel}>{props.rightLabel}</div>
            <div style={compareMiniSideValue}>{props.rightValue}</div>
          </div>
        </div>
      </div>
    );
...
const EmptyText = defineComponent({
  setup(props, { slots }) {
    return () => <div style={emptyState}>{slots.default?.()}</div>;
  },
});

const ModernBar = defineComponent({
  props: { value: {} },
  setup(props) {
    return () => {
