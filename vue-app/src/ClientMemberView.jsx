...
const ClientMemberView = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      ...
      return (
        <div class="client-member-root" style={page}>
          <style>
            {`
          @media print {
            @page {
        ...
        `}
          ...
          <section class="member-top-grid" style={topGrid}>
            <KpiCard
              icon={<AssetsIcon />}
              title="סך נכסים"
          ...
          <section class="member-lower-grid" style={lowerTwoGrid}>
            <SectionCard title="חשיפות אישיות" icon="📊">
              <div class="member-explanation" style={explanation}>
          ...
          <section class="member-compare-grid" style={compareGrid}>
          ...
          <SectionCard title="מוצרים / תוכניות" icon="📄">
            <div class="member-products-header" style={productsHeaderRow}>
              ...
              <div class="member-filter-print-hide" style={filterWrap}>
                ...
                >
                  ...
                  <span style={filterArrow}>{filterOpen.value ? "▲" : "▼"}</span>
                </button>
                  ...
                  <div style={filterDropdown}>
                    {filterOptions.value.map((option) => (
                      <button
                        ...
                        style={filterDropdownItem(option.id === productFilter.value)}
                      >
                        {option.label}
                      </button>
                    ))}
            ...
            {filteredProducts.value.length ? (
              <div class="member-table-wrap" style={tableWrap}>
                <table class="member-products-table" style={table}>
                  <thead>
                    <tr>
                      <th style={th}>מוצר</th>
...
const Header = defineComponent({
  ...
  setup(props) {
    return () => (
      <section class="member-hero" style={heroHeader}>
...
const KpiCard = defineComponent({
  ...
  setup(props) {
    return () => (
      <div class="member-kpi-card" style={kpiCard}>
        <div style={kpiIconWrap}>{props.icon}</div>
        <div style={kpiTitle}>{props.title}</div>
        <div class="member-kpi-value" style={kpiValue}>{props.value}</div>
...
const SectionCard = defineComponent({
  ...
  setup(props, { slots }) {
    return () => (
      <section class="member-section-card" style={sectionCard}>
...
const ExposureMetricBlock = defineComponent({
  props: { label: {}, value: {}, valueText: {}, description: {} },
  setup(props) {
    return () => (
      <div>
        <div style={exposureTopRow}>
          <div>
            <div style={exposureLabel}>{props.label}</div>
            <div style={exposureDescription}>{props.description}</div>
          </div>

          <div class="member-exposure-value" style={exposureValue}>{props.valueText}</div>
        </div>
...
const DataRow = defineComponent({
  ...
  setup(props) {
    return () => (
      <div style={dataRow}>
        <div style={dataLabel}>{props.label}</div>
        <div style={dataValue}>{props.value}</div>
      </div>
    );
  },
});
...
const DonutSummaryCard = defineComponent({
  props: { title: {}, subtitle: {}, items: {}, formatCurrency: {} },
  setup(props) {
    return () => {
      ...
      if (!data.segments.length) {
        return (
          <section class="member-donut-card" style={{ ...donutCard, border: "none", boxShadow: "none" }}>
      ...
      return (
        <section class="member-donut-card" style={{ ...donutCard, border: "none", boxShadow: "none" }}>
          ...
          <div class="member-donut-layout" style={donutLayout}>
            <div class="member-donut-chart" style={{ display: "flex", justifyContent: "center" }}>
...
const ExplodedDonutChart = defineComponent({
  props: { segments: {}, size: { default: 150 } },
  setup(props) {
    return () => {
      ...
      const arcSegments = props.segments.map((seg) => {
...
const ExposureBadge = defineComponent({
  props: { value: {} },
  setup(props) {
    return () => {
