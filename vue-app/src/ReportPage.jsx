...
const CapitalClassificationReportSection = defineComponent({
  props: { entries: {}, styles: {} },
  setup(props) {
    return () => {
      const safeEntries = normalizeCapitalReportArray(props.entries);
...
const CapitalClassificationOwnerBlock = defineComponent({
  props: { entry: {}, styles: {} },
  setup(props) {
    return () => {
...
const CapitalMiniStat = defineComponent({
  props: { label: {}, value: {} },
  setup(props) {
    return () => {
      const isCapital = String(props.label || "").includes("הון");
      const isPension = String(props.label || "").includes("קצבה");
      ...
      return (
        ...
        >
          ...
          <div style={{ color: "#00215D", fontSize: 14, fontWeight: 900, direction: "ltr" }}>
            {getCapitalRowValue({ value: props.value }, "value")}
          </div>
        </div>
      );
    };
...
const CapitalLegend = defineComponent({
  setup() {
    return () => (
      <div
        style={{
...
const CapitalClassificationTable = defineComponent({
  props: { title: {}, subtitle: {}, rows: {}, type: {} },
  setup(props) {
    return () => {
      ...
      const displayRows = type === "study"
        ? normalizeCapitalReportArray(props.rows).map((row) => ({
            ...row,
            studyBalance: getStudyFundBalance(row),
          }))
        : normalizeCapitalReportArray(props.rows);
      ...
      return (
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
            <div>
              <div style={{ color: "#00215D", fontSize: 18, fontWeight: 900 }}>{props.title}</div>
              <div style={{ color: "#627D98", fontSize: 12, marginTop: 4 }}>{props.subtitle}</div>
            </div>
          </div>
...
const ReportPage = defineComponent({
  ...
  setup(props) {
    ...
    const generalConversationSummary = ref(
    ...
    const summaryTopics = ref(
      createDefaultSummaryTopics(
        props.reportData?.summaryTopics || props.reportData?.conversationSummaryTopics || []
      )
    );

    const manualActionRecommendations = ref(
    ...
    const isClientLinkCopied = ref(false);

    watch(
      () => props.reportData,
      () => {
        generalConversationSummary.value =
          props.reportData?.generalConversationSummary ||
          props.reportData?.conversationSummary ||
          props.reportData?.clientConversationSummary ||
          props.reportData?.summaryText ||
        ...
        summaryTopics.value = createDefaultSummaryTopics(
          props.reportData?.summaryTopics || props.reportData?.conversationSummaryTopics || []
        );

        manualActionRecommendations.value = cleanManualActionText(
          props.reportData?.manualActionRecommendations ||
            props.reportData?.actionRecommendations ||
            props.reportData?.clientActionRecommendations ||
            props.reportData?.recommendationsText ||
    ...
    const conversationSummary = computed(() =>
      buildConversationSummaryText(generalConversationSummary.value, summaryTopics.value)
    );

    const actionRecommendations = computed(() =>
      buildActionRecommendationsText(manualActionRecommendations.value, summaryTopics.value)
    );

    const selectedSummaryTopics = computed(() =>
      summaryTopics.value.filter(
        (topic) =>
          topic.checked &&
    ...
    const reportDataForClient = computed(() => ({
      ...safeReportData.value,
      generalConversationSummary: generalConversationSummary.value,
      summaryTopics: summaryTopics.value,
      conversationSummary: conversationSummary.value,
      clientConversationSummary: conversationSummary.value,
      summaryText: conversationSummary.value,
      manualActionRecommendations: manualActionRecommendations.value,
      actionRecommendations: actionRecommendations.value,
      clientActionRecommendations: actionRecommendations.value,
      recommendationsText: actionRecommendations.value,
    ...
    const retirementLumpBars = computed(() => {
      const family = safeReportData.value.family || {};
      ...
      return [
        { label: "עם הפקדות", value: withDeposits, display: formatCurrency(withDeposits), ratio: (withDeposits / maxValue) * 100, tone: "primary" },
        { label: "ללא הפקדות", value: withoutDeposits, display: formatCurrency(withoutDeposits), ratio: (withoutDeposits / maxValue) * 100, tone: "muted" },
      ];
    });

    const retirementPensionBars = computed(() => {
      const family = safeReportData.value.family || {};
    ...
    const handleCreateClientLink = async () => {
      if (!props.reportData || !props.reportData.family) {
        alert("אין דוח מוכן ליצירת לינק. קודם יש להפיק דוח.");
        return;
      }

      saveClientDashboardData(reportDataForClient.value);

      const result = props.onCreateShareLink({
        expirationHours: 24,
        reportData: reportDataForClient.value,
        clientModel: buildClientModelFromReportData(reportDataForClient.value),
        conversationSummary: conversationSummary.value,
        actionRecommendations: actionRecommendations.value,
        recommendationsText: actionRecommendations.value,
      });
      ...
      isClientLinkCopied.value = true;
      window.setTimeout(() => (isClientLinkCopied.value = false), 3500);
    };

    const updateSummaryTopic = (id, field, value) => {
      summaryTopics.value = summaryTopics.value.map((topic) =>
    ...
    const markAllSummaryTopics = () => {
      summaryTopics.value = summaryTopics.value.map((topic) => ({ ...topic, checked: true }));
    };

    const clearAllSummaryTopicMarks = () => {
      summaryTopics.value = summaryTopics.value.map((topic) => ({ ...topic, checked: false }));
    };
  ...
  const styles = {
    ...
    return () => {
      ...
      const recognizedPensionEntries = normalizeRecognizedPensionReportData(safeReportData.value);
      const hasVestedBalanceTable = recognizedPensionEntries.length > 0;

      const section28CappingEntries = normalizeSection28CappingReportData(safeReportData.value);
      const hasSection28Capping = section28CappingEntries.length > 0;

      const capitalClassificationEntries = normalizeCapitalClassificationReportData(safeReportData.value);
...
`}
          ...
          <div class="screen-report-root" style={styles.page}>
            <div class="no-print" style={styles.actionsBar}>
              ...
              <button onClick={props.onBack} class="action-button">
                חזרה למסך העלאה
              </button>
            </div>
            ...
            <div style={styles.container}>
              ...
              >
                <div class="responsive-hero-logo" style={styles.heroLogoWrap}>
                  <ZviranLogo light />
                </div>
                ...
                <div class="responsive-hero-meta" style={styles.heroMeta}>
                  <div style={styles.heroClientLogoBox}>
                    {props.reportData?.clientLogo ? (
                      <img
                        src={props.reportData.clientLogo}
                        alt="לוגו חברה"
              ...
              >
                ...
                <ComparisonChartCard
                  ...
                  bars={retirementPensionBars.value}
                />
              </section>
              ...
              >
                ...
                <section class="equity-print avoid-break" style={styles.equityCard}>
                  <div style={styles.sectionHeader}>
                    <div style={styles.titleWithIcon}>
                      <span>📊</span>
              ...
              >
                ...
                <div class="responsive-members-grid" style={styles.membersGrid}>
                ...
                <div style={styles.recommendationsWrap}>
                  <div class="screen-only">
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <textarea
                        value={generalConversationSummary.value}
                        onInput={(e) => (generalConversationSummary.value = e.target.value)}
                    ...
                    <div style={styles.summaryFlowToolbar}>
                      ...
                      <div style={{ color: textSoft, fontSize: 12, fontWeight: 800 }}>
                        נבחרו {selectedSummaryTopics.value.length} נושאים להצגה ללקוח
                      </div>
                    </div>

                    <div style={styles.summaryTopicList}>
                      {summaryTopics.value.map((topic) => (
                        <div
                          key={topic.id}
                        ...
                        >
                          ...
                          {topic.checked ? (
                            <div class="summary-topic-grid" style={styles.summaryTopicGrid}>
                              <div>
                    ...
                    <div style={styles.summaryPreviewBox}>
                      {conversationSummary.value || "אין סיכום להצגה ללקוח כרגע."}
                    </div>
                  </div>
                </div>
              </section>
              ...
              >
                ...
                <div style={styles.recommendationsWrap}>
                  <div class="screen-only">
                    ...
                    <textarea
                      value={manualActionRecommendations.value}
                      onInput={(e) => (manualActionRecommendations.value = e.target.value)}
                      style={styles.recommendationsText}
                    ...
                    <div style={{ marginTop: 14 }}>
                      ...
                      <div style={styles.summaryPreviewBox}>
                        {actionRecommendations.value || "אין פעולות להצגה ללקוח כרגע."}
                      </div>
                    </div>
                  </div>
                </div>
...
const Section28CappingReport = defineComponent({
  props: { data: {}, styles: {} },
  setup(props) {
    return () => {
      const groups = Array.isArray(props.data?.groups) ? props.data.groups : [];
      const comparisonRows = Array.isArray(props.data?.comparisonRows)
        ? props.data.comparisonRows
        : [];
      ...
      return (
        <div>
          {costGroup ? <Section28CostSplit group={costGroup} styles={props.styles} /> : null}

          {savingGroup ? (
            <div style={{ marginTop: 14 }}>
              <Section28SavingSimulation group={savingGroup} styles={props.styles} />
            </div>
          ) : null}

          {comparisonRows.length ? (
            <Section28ComparisonTable rows={comparisonRows} styles={props.styles} />
          ) : null}

          {retirementGroup ? (
            <div style={{ marginTop: 14 }}>
              <Section28RetirementSimulation group={retirementGroup} styles={props.styles} />
            </div>
          ) : null}

          {otherGroups.length ? (
            <div class="section-28-grid-print" style={{ ...props.styles.section28Grid, marginTop: 14 }}>
              {otherGroups.map((group) => (
                <Section28Group key={group.id || group.title} group={group} styles={props.styles} />
              ))}
            </div>
          ) : null}
        </div>
...
const Section28CostSplit = defineComponent({
  ...
  setup(props) {
    return () => {
      ...
      const rows = Array.isArray(props.group?.rows) ? props.group.rows.filter((row) => isMeaningfulSection28Value(row.value)) : [];
...
const Section28Group = defineComponent({
  props: { group: {}, styles: {} },
  setup(props) {
    return () => {
      const styles = props.styles;
      const rows = Array.isArray(props.group?.rows)
        ? props.group.rows.filter((row) => isMeaningfulSection28Value(row.value))
        : [];
      ...
      return (
        <div style={styles.section28Group}>
          <div style={styles.section28GroupTitle}>{props.group.title}</div>
...
const Section28DataRow = defineComponent({
  props: { row: {}, styles: {}, isLast: { default: false }, forceHighlight: { default: false } },
  setup(props) {
    return () => {
      const styles = props.styles;
      const row = props.row;
      const isHighlighted = props.forceHighlight || isSection28ImportantRow(row.label);
      ...
      const rowStyle = isHighlighted
        ...
        : {
            ...styles.section28Row,
            borderBottom: props.isLast ? "none" : styles.section28Row.borderBottom,
          };
    ...
    return () => (
      ...
      >
        ...
        <div style={{ fontSize: 14, fontWeight: 900, direction: "ltr" }}>
          {formatSection28DisplayValue(props.row.value)}
        </div>
      </div>
    );
  },
...
const VestedPdfCalculationTable = defineComponent({
  ...
  setup(props) {
    return () => {
      const styles = props.styles;
      const pdfRows = Array.isArray(props.rows) ? props.rows : [];
...
const ManualRecognizedPensionTable = defineComponent({
  ...
  setup(props) {
    return () => {
      const styles = props.styles;
      const manualTotal = props.rows.reduce(
...
const VestedBalanceTable = defineComponent({
  props: { table: {}, adjustments: {}, styles: {} },
  setup(props) {
    return () => {
      const styles = props.styles;
      const pdfRows = Array.isArray(props.table?.rows) ? props.table.rows : [];
      const manualRows = getManualRecognizedPensionRows(props.adjustments);
...
const KpiCard = defineComponent({
  props: { styles: {}, icon: {}, title: {}, value: {}, subtext: {} },
  setup(props) {
    return () => (
      <div style={props.styles.kpiCard} class="kpi-card-hover">
        <div
        ...
        >
          <div style={props.styles.kpiIconWrap}>{props.icon}</div>
          <div style={props.styles.kpiTitle}>{props.title}</div>
          <div style={props.styles.kpiValue}>{props.value}</div>
          <div style={props.styles.kpiSub}>{props.subtext}</div>
        </div>
      </div>
    );
  },
...
const ComparisonChartCard = defineComponent({
  props: { styles: {}, title: {}, explanation: {}, bars: {} },
  setup(props) {
    return () => {
      const styles = props.styles;
      return (
        <div style={styles.compareCard}>
          <div style={styles.compareTitle}>{props.title}</div>
          <div style={styles.compareDesc}>{props.explanation}</div>
...
const EquityBarModern = defineComponent({
  props: { value: {} },
  setup(props) {
    return () => {
      const safeValue = Math.max(0, Math.min(100, Number(props.value || 0)));
...
const Donut3D = defineComponent({
  props: { gradient: {}, size: { default: 110 }, hole: { default: "30%" }, soft: { type: Boolean, default: false } },
  setup(props) {
    return () => {
      ...
      return (
        <div
          style={{
            ...
            height: `${size}px`,
            borderRadius: "50%",
            background: `conic-gradient(${props.gradient})`,
            position: "relative",
            flexShrink: 0,
            boxShadow: soft
...
const DonutSummaryCard = defineComponent({
  props: { title: {}, subtitle: {}, items: {}, colors: {}, styles: {}, formatCurrency: {} },
  setup(props) {
    return () => {
      const styles = props.styles;
      const { segments, gradient } = buildDonutSegments(props.items, props.colors);

      return (
        <section style={styles.donutCard}>
          <h3 style={styles.donutTitle}>{props.title}</h3>
          <div style={{ ...styles.smallText, marginTop: "6px" }}>{props.subtitle}</div>
                ...
                segments.slice(0, 5).map((seg, index) => (
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0, textAlign: "right" }}>
                      ...
                      >
                        {props.formatCurrency(seg.value)}
                      </div>
                    </div>
...
const DonutBreakdownCard = defineComponent({
  props: {
    title: { default: "חלוקה עבור אפיקים ראשיים" },
    subtitle: { default: "התרשים מציג את חלוקת אפיקי ההשקעה בתיק ביחס לסך הנכסים." },
    items: {},
    formatCurrency: {},
    colors: {},
  },
  setup(props) {
    return () => {
      ...
      const { segments, total, gradient } = buildDonutSegments(props.items, colors);
...
const PercentDonutCard = defineComponent({
  props: { title: {}, subtitle: {}, items: {}, colors: {}, styles: {} },
  setup(props) {
    return () => {
      const styles = props.styles;
...
const ZviranLogo = defineComponent({
    ...
    return () => (
      <div
        style={{
          ...
          gap: "12px",
          direction: "ltr",
          justifyContent: props.light ? "flex-end" : "flex-start",
        }}
      >
        <div
          style={{
            ...
            height: "54px",
            borderRadius: "50%",
            background: props.light ? "rgba(255,255,255,0.14)" : "#0A2668",
            border: props.light ? "1px solid rgba(255,255,255,0.25)" : "none",
            position: "relative",
            flexShrink: 0,
          }}
        >
        ...
        <div style={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
          ...
          <div
            style={{
              marginTop: "6px",
              fontSize: "12px",
              color: props.light ? "rgba(255,255,255,0.8)" : "#6B7A99",
              letterSpacing: "0.4px",
            }}
          >
...
const PrintReportA4 = defineComponent({
  props: { reportData: {}, conversationSummary: { default: "" }, actionRecommendations: { default: "" } },
  setup(props) {
    return () => {
      const data = props.reportData || {};
      const family = data.family || {};
      const printConversationSummary =
        props.conversationSummary || data.conversationSummary || data.clientConversationSummary || data.summaryText || "";
      const printActionRecommendations =
        props.actionRecommendations || data.actionRecommendations || data.clientActionRecommendations || data.recommendationsText || data.recommendations || "";
  ...
  `;
      ...
      const PrintHeader = ({ title, page }) => (
        <>
          <div class="print-page-header">
            <h2 class="print-page-title">{title}</h2>
            <div class="print-logo-text">zviran</div>
          </div>
          <div class="print-footer"><span>Zviran · Total Rewards Experts</span><span>עמוד {page}</span></div>
        </>
      );
      ...
      const Kpi = ({ label, value, note }) => (
        <div class="print-card">
          ...
          {note ? <div class="print-muted" style={{ marginTop: 6 }}>{note}</div> : null}
        </div>
      );
      ...
      const PieBreakdown = ({ title, items, large = false, compactClassName = "" }) => {
        ...
        return (
          <div class={`print-card ${compactClassName}`} style={{ minHeight: large ? "112mm" : "auto" }}>
            <h3 class="print-section-heading">{title}</h3>
            <div style={{ display: "grid", gridTemplateColumns: large ? "1fr 58mm" : "1fr 45mm", gap: "4mm", alignItems: "center" }}>
              <div>
                {segments.slice(0, large ? 10 : 6).map((item, index) => (
                  ...
                  >
                    <div
                      ...
                      title={item.name}
                    >
                      <span class="print-swatch" style={{ background: item.color }} />
                      {item.name}
                    </div>
                ...
                {!segments.length ? <div class="print-muted">אין נתונים להצגה</div> : null}
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "3mm" }}>
                <div class="print-pie" style={{ width: large ? "56mm" : "42mm", height: large ? "56mm" : "42mm", background: `conic-gradient(${gradient})` }} />
              </div>
            </div>
          </div>
        );
      ...
      const ExposureCard = ({ label, value }) => {
        ...
        return (
          <div class="print-exposure-card">
            <div class="print-exposure-row">
              <div class="print-kpi-label" style={{ marginBottom: 0 }}>{label}</div>
              <div class="print-kpi-value">{fmtPercent(value)}</div>
            </div>

            <div class="print-bar-track">
              <div class="print-bar-fill" style={{ width: `${safeValue}%` }} />
            </div>

            <div class="print-exposure-legend">
              <span><span class="print-exposure-dot" style={{ background: "linear-gradient(90deg, #FF2756, #00215D)" }} />שיעור החשיפה</span>
              <span><span class="print-exposure-dot" style={{ background: "#EAF1FB" }} />יתרה עד 100%</span>
            </div>
          </div>
        );
      };
          ...
          <div class="print-card">
            ...
            {[
              ...
              <div key={row.label} style={{ marginBottom: 12 }}>
                ...
                <div class="print-bar-track"><div class="print-bar-fill" style={{ width: `${Math.max((row.value / maxValue) * 100, row.value ? 5 : 0)}%` }} /></div>
              </div>
            ))}
          </div>
        );
      ...
      const renderSingleSection28PrintSummary = (entry) => {
        ...
        const printRow = (row, index, forceHighlight = false) => {
          ...
          return (
            <div
              class={`print-section28-line${highlight ? " print-section28-line-highlight" : ""}`}
              key={`${row.label}-${index}`}
            >
              <div class="print-section28-line-label">{row.label}</div>
              <div class="print-section28-line-value">{formatSection28DisplayValue(row.value)}</div>
            </div>
          );
        };

        const renderSide = (title, rows, summaryRows) => (
          <div class="print-section28-side-card">
            <div class="print-section28-side-title">{title}</div>
            {rows.map((row, index) => printRow(row, index))}
            {summaryRows.map((row, index) => printRow(row, index, true))}
            {!rows.length && !summaryRows.length ? <div class="print-muted">אין נתון להצגה</div> : null}
          </div>
        );
        ...
        const renderSimpleGroup = (group, titleOverride, limit = 4) => {
          ...
          return (
            <div class="print-card-soft" style={{ marginTop: "3mm" }}>
        ...
        return (
          <div class="print-card-soft" style={{ marginBottom: "4mm" }}>
            ...
            <div class="print-card-soft" style={{ marginBottom: "3mm" }}>
              <div style={{ color: "#00215D", fontWeight: 900, fontSize: 10.5, marginBottom: "2mm" }}>
                פירוט עלויות עובד / מעסיק
              </div>

              <div class="print-section28-two-cols">
                {renderSide("חלק מעסיק", employerRows, employerSummaryRows)}
                {renderSide("חלק עובד", employeeRows, employeeSummaryRows)}
              </div>

              {monthlyRow ? (
                <div class="print-section28-monthly">
                  <div class="print-section28-monthly-label">{monthlyRow.label}</div>
                  <div class="print-section28-monthly-value">{formatSection28DisplayValue(monthlyRow.value)}</div>
                </div>
              ) : null}
            </div>
      ...
      const Section28PrintSummary = () => {
        if (!hasSection28Capping) {
          return <div class="print-muted">לא קיימים נתוני סעיף 28 בדוח.</div>;
        }

        return (
          <div>
            {section28CappingEntries.map((entry, index) => (
              <Fragment key={`${entry.owner || "owner"}-${entry.sourceFileName || index}`}>
      ...
      const RecognizedPensionPrintSummary = () => (
        <div>
          {recognizedPensionEntries.map((entry, entryIndex) => {
            ...
            return (
              <div class="print-card-soft" style={{ marginBottom: "3mm" }} key={`${entry.owner || "owner"}-${entryIndex}`}>
                ...
                {vestedRows.length ? (
                  <div style={{ marginBottom: "3mm" }}>
                    ...
                    <table class="print-table" style={{ marginTop: "3mm" }}>
          ...
          {!recognizedPensionEntries.length ? <div class="print-muted">לא קיימים נתוני קצבה מוכרת בדוח.</div> : null}
        </div>
      );
      ...
      const PrintSummaryActionsPage = ({ page }) => (
        <section class="print-a4-page print-summary-actions-page">
          {PrintHeader({ title: "סיכום שיחה ופעולות אופרטיביות", page })}

          <div class="print-card" style={{ marginBottom: "5mm" }}>
            <h3 class="print-section-heading">סיכום שיחה</h3>
            {printSummaryBlocks.length ? (
              ...
              <div class="print-muted">לא הוזן סיכום בדוח.</div>
            )}
          </div>

          <div class="print-card">
            <h3 class="print-section-heading">פעולות אופרטיביות לביצוע</h3>
            {printActionBlocks.length ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "3mm" }}>
                {printActionBlocks.map((block, blockIndex) => (
                  <div key={`action-print-block-${blockIndex}`} class="print-card-soft" style={{ borderColor: "#E2D1BF" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "1.8mm" }}>
                      {block.lines.length ? (
                        ...
                        <div class="print-muted">לא הוזנו פעולות לנושא זה.</div>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div class="print-muted">לא הוזנו המלצות לפעולה בדוח.</div>
            )}
          </div>
        </section>
      );
      ...
      const PrintCapitalStat = ({ label, value }) => (
        <div class="print-capital-stat">
          <div class="print-capital-stat-label">{label}</div>
          <div class="print-capital-stat-value">{getCapitalRowValue({ value }, "value")}</div>
        </div>
      );
      ...
      const PrintCapitalTable = ({ title, rows, type }) => {
        ...
        return (
          <div>
            {type !== "study" ? (
              <div class="print-capital-legend">
                <span><i class="legend-capital" /> כספים הוניים / נזילים / תגמולים עד 1.1.2000</span>
                <span><i class="legend-pension" /> כספים קצבתיים</span>
              </div>
            ) : null}
            <div class="print-capital-table-title">{title}</div>
            <table class="print-capital-table">
              <thead>
                <tr>{columns.map((column) => <th key={column.key} class={`tone-${getCapitalCellTone(column)}`}>{column.label}</th>)}</tr>
              </thead>
              <tbody>
                {displayRows.slice(0, type === "study" ? 8 : 7).map((row, rowIndex) => (
                  <tr key={row.id || `${row.policyNumber || row.planName || "row"}-${rowIndex}`}>
                    {columns.map((column) => (
                      <td key={column.key} class={`tone-${type === "study" ? "neutral" : getCapitalCellTone(column)}`}>
                        {getCapitalDisplayValue(row, column.key)}
                      </td>
                    ))}
                  </tr>
                ))}
                <tr class="print-capital-total">
      ...
      const PrintCapitalClassificationPage = () => (
        <section class="print-a4-page print-capital-page">
          {PrintHeader({ title: "פירוט פוליסות וקרנות", page: capitalClassificationPageNumber })}
          <div class="print-muted" style={{ marginBottom: "3mm" }}>
          ...
          {capitalClassificationEntries.map((entry, entryIndex) => {
            ...
            return (
              <div class="print-capital-owner" key={`${entry.owner}-${entryIndex}`}>
                <div class="print-capital-owner-header">
                  <div>
                    <div class="print-capital-owner-title">{entry.ownerLabel || "בן/בת זוג"}</div>
                    <div class="print-capital-owner-source">{entry.sourceFileName ? `מקור הנתונים: ${entry.sourceFileName}` : "נתוני סיווג כספים"}</div>
                  </div>
                  <div class="print-capital-stats">
          ...
          <div class="print-footer"><span>Zviran · Total Rewards Experts</span><span>עמוד {capitalClassificationPageNumber}</span></div>
        </section>
      );
        ...
        <div class="print-report-root" aria-hidden="true">
          ...
          <section class="print-a4-page">
            <div class="print-cover-top">
              ...
              <h1 class="print-cover-title">דוח פנסיוני משפחתי מאוחד</h1>
              <div class="print-cover-subtitle">
            ...
            <div class="print-cover-body">
              <div>
                <h3 class="print-section-heading">נתונים מרכזיים</h3>
                <div class="print-cover-kpis">
              ...
              <div class="print-cover-pies">
                {PieBreakdown({ title: "חלוקה לפי מוצרים", items: products })}
                {PieBreakdown({ title: "חלוקה לפי גופים מנהלים", items: managers })}
              </div>
            </div>

            <div class="print-footer"><span>Zviran · Total Rewards Experts</span><span>עמוד 1</span></div>
          </section>

          <section class="print-a4-page">
            {PrintHeader({ title: "אפיקים ראשיים וחשיפות", page: 2 })}

            <div class="print-page-2-main">
              {PieBreakdown({ title: "חלוקה עבור אפיקים ראשיים", items: mainGroups, large: true })}
            </div>

            <div class="print-page-2-exposures">
              {PieBreakdown({ title: 'פירוט חו"ל / ישראל', items: foreignExposureAllocation, compactClassName: "print-pie-card" })}

              <div class="print-exposure-stack">
          ...
          {shouldShowPensionAppendixPage ? (
            <section class="print-a4-page print-appendix-page">
              {PrintHeader({ title: "סעיף 28 וקצבה מוכרת לפי בן/בת זוג", page: appendixPageNumber })}
              <div class="print-appendix-grid">
                <div class="print-appendix-card print-appendix-block">
                  <h3 class="print-section-heading">קיטום על פי סעיף 28 לפי בן/בת זוג</h3>
                  {Section28PrintSummary()}
                </div>
                <div class="print-appendix-card print-appendix-block">
                  <h3 class="print-section-heading">צבירה מוכרת / קצבה מוכרת לפי בן/בת זוג</h3>
          ...
          {memberPages.map((pageMembers, pageIndex) => (
            <section class="print-a4-page" key={`members-page-${pageIndex}`}>
              {PrintHeader({ title: "פירוט לפי בני משפחה", page: firstMemberPageNumber + pageIndex })}
              <div class="print-grid-2">
                {pageMembers.map((member, index) => (
                  <div class="print-card" key={member.id || member.name || index}>
                    <h3 class="print-section-heading">{member.name || "ללא שם"}</h3>
                    <div class="print-grid-2">
          ...
          <section class="print-a4-page">
            ...
            <div class="print-grid-2" style={{ marginBottom: "5mm" }}>
              <div class="print-card">
                <h3 class="print-section-heading">הלוואות על חשבון מוצרים פנסיוניים</h3>
                <div class="print-grid-2" style={{ marginBottom: 12 }}>
                  {Kpi({ label: "סה״כ סכום הלוואות", value: fmtCurrency(totalLoansAmount) })}
                  {Kpi({ label: "יתרת הלוואות", value: fmtCurrency(totalLoansBalance) })}
                </div>
                {loanDetails.length ? <table class="print-table"><thead><tr><th>שם</th><th>סכום</th><th>יתרה</th><th>תדירות</th><th>סיום</th></tr></thead><tbody>{loanDetails.slice(0, 10).map((loan, index) => <tr key={loan.id || index}><td>{[loan.firstName, loan.familyName].filter(Boolean).join(" ") || "—"}</td><td>{fmtCurrency(loan.amount)}</td><td>{fmtCurrency(loan.balance)}</td><td>{loan.repaymentFrequency || "—"}</td><td>{fmtDate(loan.endDate)}</td></tr>)}</tbody></table> : <div class="print-muted">לא התקבל פירוט הלוואות להצגה.</div>}
              </div>
              <div class="print-card">
                <h3 class="print-section-heading">סיכום מהיר</h3>
                <div class="print-grid-2" style={{ marginBottom: 12 }}>
