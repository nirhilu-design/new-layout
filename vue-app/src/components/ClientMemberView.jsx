...
const ClientMemberView = defineComponent({
  name: "ClientMemberView",
  ...
  setup(props) {
    ...
    const filterOptions = computed(() => {
      const products = props.member?.products || [];
    ...
    const selectedFilterLabel = computed(
      () =>
        filterOptions.value.find((option) => option.id === productFilter.value)
          ?.label || "כל המוצרים"
    );
    ...
    const filteredProducts = computed(() => {
      ...
      const [type, value] = productFilter.value.split(":");
    ...
    return () => {
      ...
      if (!member) {
        return <div style={px({ direction: "rtl" })}>לא נמצא בן משפחה</div>;
      }
      ...
      return (
        <div class="client-member-root" style={px(page)}>
          <style>
            {`
          @media print {
            @page {
        ...
        `}
          ...
          <section class="member-top-grid" style={px(topGrid)}>
            <KpiCard
              icon={<AssetsIcon />}
              title="סך נכסים"
          ...
          <section class="member-lower-grid" style={px(lowerTwoGrid)}>
            <SectionCard title="חשיפות אישיות" icon="📊">
              <div class="member-explanation" style={px(explanation)}>
          ...
          <section class="member-compare-grid" style={px(compareGrid)}>
          ...
          <SectionCard title="מוצרים / תוכניות" icon="📄">
            <div class="member-products-header" style={px(productsHeaderRow)}>
              <div>
                <div style={px(productsHeaderTitle)}>פירוט מוצרים / תוכניות</div>
                <div style={px(productsHeaderSub)}>ניתן לסנן לפי סוג מוצר.</div>
              </div>

              <div class="member-filter-print-hide" style={px(filterWrap)}>
                <button
                  type="button"
                  onClick={() => (filterOpen.value = !filterOpen.value)}
                  style={px(filterButton)}
                >
                  <span style={px(hamburgerIcon)}>☰</span>
                  <span style={px(filterButtonText)}>{selectedFilterLabel.value}</span>
                  <span style={px(filterArrow)}>{filterOpen.value ? "▲" : "▼"}</span>
                </button>

                {filterOpen.value && (
                  <div style={px(filterDropdown)}>
                    {filterOptions.value.map((option) => (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => handleFilterSelect(option.id)}
                        style={px(filterDropdownItem(option.id === productFilter.value))}
                      >
                        {option.label}
                      </button>
                    ))}
            ...
            {filteredProducts.value.length ? (
              <div class="member-table-wrap" style={px(tableWrap)}>
                <table class="member-products-table" style={px(table)}>
                  <thead>
                    <tr>
                      <th style={px(th)}>מוצר</th>
                      <th style={px(th)}>גוף מנהל</th>
                      <th style={px(th)}>סוג מוצר</th>
                      <th style={px(th)}>מספר פוליסה</th>
                      <th style={px(th)}>צבירה</th>
                      <th style={px(th)}>הפקדה חודשית</th>
                      <th style={px(th)}>קצבה צפויה</th>
                      <th style={px(th)}>דמי ניהול מצבירה</th>
                      <th style={px(th)}>חשיפה מנייתית</th>
                      <th style={px(th)}>חשיפה לחו"ל</th>
                    </tr>
                  </thead>
                  ...
                  <tbody>
                    {filteredProducts.value.map((product) => (
                      <tr key={product.id}>
                        <td style={px(td)}>{product.planName}</td>
                        <td style={px(td)}>{product.managerName}</td>
                        <td style={px(td)}>{product.productType}</td>
                        <td style={px(td)}>{product.policyNo || "—"}</td>
                        <td style={px(td)}>{formatCurrency(product.currentValue)}</td>
                        <td style={px(td)}>{formatCurrency(product.monthlyDeposit)}</td>
                        <td style={px(td)}>
...
function Header({ eyebrow, title, subtitle }) {
  return (
    <section class="member-hero" style={px(heroHeader)}>
...
function KpiCard({ icon, title, value, subtext }) {
  return (
    <div class="member-kpi-card" style={px(kpiCard)}>
      <div style={px(kpiIconWrap)}>{icon}</div>
      <div style={px(kpiTitle)}>{title}</div>
      <div class="member-kpi-value" style={px(kpiValue)}>{value}</div>
...
const SectionCard = (props, { slots }) => {
  ...
  return (
    <section class="member-section-card" style={px(sectionCard)}>
...
function ExposureMetricBlock({ label, value, valueText, description }) {
  return (
    <div>
      <div style={px(exposureTopRow)}>
        <div>
          <div style={px(exposureLabel)}>{label}</div>
          <div style={px(exposureDescription)}>{description}</div>
        </div>

        <div class="member-exposure-value" style={px(exposureValue)}>{valueText}</div>
      </div>
...
function DataRow({ label, value }) {
  return (
    <div style={px(dataRow)}>
      <div style={px(dataLabel)}>{label}</div>
      <div style={px(dataValue)}>{value}</div>
    </div>
  );
}
...
function DonutSummaryCard({ title, subtitle, items, formatCurrency }) {
  ...
  if (!data.segments.length) {
    return (
      <section class="member-donut-card" style={px({ ...donutCard, border: "none", boxShadow: "none" })}>
  ...
  return (
    <section class="member-donut-card" style={px({ ...donutCard, border: "none", boxShadow: "none" })}>
      ...
      <div class="member-donut-layout" style={px(donutLayout)}>
        <div class="member-donut-chart" style={px({ display: "flex", justifyContent: "center" })}>
          <ExplodedDonutChart segments={data.segments} size={150} />
        </div>

        <div style={px(legendList)}>
...
function ExplodedDonutChart({ segments, size = 150 }) {
  ...
  return (
    <svg
      ...
      style={px({ display: "block", overflow: "visible" })}
    >
      <defs>
...
function ExposureBadge({ value }) {
  ...
  return (
    <span style={px({ color, fontWeight: 800 })}>
