...
// קריאת ה־Excel נעשית מקומית: ספריית SheetJS (xlsx) מצורפת ל־bundle של
// האפליקציה (dependency מקומי) במקום להיטען מ־CDN חיצוני. הייבוא הדינמי
// משאיר את הספרייה מחוץ ל־chunk הראשי כך שהיא נטענת רק כשקולטים קובץ Excel.
...
const extractCapitalClassificationFromExcel = async (file, owner) => {
  ...
  const normalizedRows = rows
    ...
    .filter((row) => {
      ...
      // מוצר / מספר קופה). The file's own summary row (=SUM(...)) and the legend
      // rows below the table have amounts or notes but no identity, so requiring
      // an identity keeps them from being counted as policies and double-counting
      // the "סה\"כ קופה" total.
